...
async function registerShop(formData) {
    ...
    let response = await fetch(`./db/shop/registerShop.php`, {
        method: "POST",
        body: formData
    });
    ...
    showAlert(
        ...
        !response.ok ? 'Tente novamente mais tarde ou entre em contato com o suporte para mais detalhes' : '',
        !response.ok ? 2000 : 1500
    );
}
async function registerClient(formData) {
    let response = await fetch(`./db/client/registerClient.php`,{
        method: "POST",
        body: formData
    });
...
async function verifyCode(email, body) {
    ...
    let response = await fetch(`./db/verifyCode.php`,{
        method: "POST",
        body: formData
    });
    let data = await response.json();
    ...
    if(!data.status){
        showAlert('error', 'Erro ao confirmar a conta!', 'Entre em contato com o suporte!', 2000);
    }
